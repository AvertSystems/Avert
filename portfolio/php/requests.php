...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $host = gethostbynamel("169.72.70.216.host.secureserver.net");
  $db_user = "d1izjj7iuecf";
  $password = "AVERTthisdb2";
  $db_name = "testdb";
  $port = "3306";
  $mysqli = new mysqli('169.72.70.216.host.secureserver.net', 'd1izjj7iuecf', 'AVERTthisdb2', 'testdb', '2224');
  // Ping db server to check connection
  if ($mysqli -> ping()) {
  ...
  if ($mysqli -> connect_errno)
        {
                echo "Could not connect to MySQL db: " . $mysqli -> connect_errno;
                exit();
        }
  //$mysqli_select_db($con, 'testdb');
  $sql = 'INSERT INTO requests (id, firstname, lastname, email, phone, description) VALUES ('', "'. $firstname .'", "'. $lastname .'", "'. $email .'", "'. $phone .'", "'. $description .'")';
